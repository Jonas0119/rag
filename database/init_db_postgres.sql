-- RAG 智能问答系统数据库初始化脚本
-- PostgreSQL (Supabase)
-- 注意：PostgreSQL 语法与 SQLite 的差异已调整

-- ==================== 用户表 ====================
CREATE TABLE IF NOT EXISTS users (
    user_id SERIAL PRIMARY KEY,  -- PostgreSQL 使用 SERIAL 替代 AUTOINCREMENT
...
CREATE TABLE IF NOT EXISTS messages (
    message_id SERIAL PRIMARY KEY,  -- PostgreSQL 使用 SERIAL
...
CREATE TABLE IF NOT EXISTS documents (
    ...
    filepath VARCHAR(500) NOT NULL,  -- Supabase Storage 路径
    file_size INTEGER NOT NULL,
    file_type VARCHAR(50) NOT NULL,
    page_count INTEGER,
    chunk_count INTEGER DEFAULT 0,
    vector_collection VARCHAR(100),  -- Pinecone index name (单 Index 共享)
